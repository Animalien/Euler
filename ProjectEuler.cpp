...
public:
  ...
  void FindPrimes(BigInt windowSize = 128)
  {
    ...
    assert(
        std::find(m_windowFlags.begin(), m_windowFlags.end(), false)
        == m_windowFlags.end());
    m_windowOffset = 0;
    ...
    for (BigInt currPossiblePrime = m_windowBase;
         currPossiblePrime < maxPossiblePrime;
         ++currPossiblePrime)
    {
...
private:
  void MarkNonPrimes(BigInt num)
  {
    ...
    for (BigInt currNonPrime = firstNonPrime; currNonPrime < maxNonPrime;
         currNonPrime += num)
    {
...
void TestPrimeFinder(BigInt window)
{
  ...
  s_primeFinder.FindPrimes(window);
  printf(
      "Prime finder window of %lld more resulted in these primes:  ", window);
  s_primeFinder.PrintPrimes();
  printf("\n\n");
}
...
public:
  ...
  void SetToDivision(
      const HugeInt& numer, BigInt denom, BigInt* remainder = nullptr)
  {
...
private:
  ...
  {
  public:
    ConstIterator(const HugeInt& parent, bool lowToHigh = true)
        : m_string(parent.m_string)
    {
  ...
  {
  public:
    ListIterator(const HugeInt* itemList, BigInt numItems)
        : m_ptr(itemList), m_ptrptr(nullptr), m_num(numItems)
    {
    }
    ListIterator(const HugeInt* const* itemList, BigInt numItems)
        : m_ptr(nullptr), m_ptrptr(itemList), m_num(numItems)
    {
    }
  ...
  void CalcSum(ListIterator& listIter)
  {
    ...
    for (;;)
    {
      ...
      for (auto iterListIter = iterList.begin(); iterListIter != iterList.end();
           ++iterListIter)
      {
...
{
  printf(
      "Sum of multiples of %d and %d under %d = %d\n",
      num1,
      num2,
      max,
      CalcSumMultiples(num1, num2, max));
}
...
{
  printf(
      "Sum of even Fibonacci numbers below %u = %u\n",
      max,
      CalcEvenFibonacciSum(max));
}
...
{
  printf(
      "Smallest multiple of all numbers from 1 to %lld = %lld\n",
      max,
      CalcSmallestMultiple(max));
}
...
{
  printf(
      "Sum square difference of first %lld natural numbers = %lld\n",
      max,
      CalcSumSquareDifference(max));
}
...
static BigInt       s_largestGridGrid[s_largestGridGridSize]
                               [s_largestGridGridSize] = {
    ...
    BigInt sliceLength)
{
  ...
  for (BigInt i = 0; i < sliceLength; ++i)
  {
    ...
    if (((BigInt)deque.size() >= sequenceLength) && (numZeroes <= 0)
    ...
    BigInt numSlices)
{
  ...
  {
    const BigInt currProduct = CalcLargestGridProduct(
        sequenceLength, xPos, yPos, xDelta, yDelta, sliceLength);
...
BigInt CalcLargestGridProduct(BigInt sequenceLength)
{
  ...
  product = CalcLargestGridProduct(
      ...
      s_largestGridGridSize,
      -1,
      s_largestGridGridSize - sequenceLength + 1);
  if (product > largestProduct)
  {
    largestProduct = product;
  }
...
{
  printf(
      "Largest grid product of length %lld = %lld\n",
      sequenceLength,
      CalcLargestGridProduct(sequenceLength));
}
...
BigInt CalcFirstHighlyDivTriNumber(BigInt moreThanNumDivisors, bool verbose)
{
  ...
  for (;;)
  {
    ...
    {
      printf(
          "\nFound more than %lld divisors (%lld).  Prime factors:  ",
          moreThanNumDivisors,
          numDivisors);
      f.PrintFactors();
      printf("\n");
...
{
  const HugeInt sum = HugeInt::GetCalcedSum(
      s_largeSumTable, sizeof(s_largeSumTable) / sizeof(s_largeSumTable[0]));
  printf(
      "Sum of huge numbers in table = %s (total num digits = %lld)\n",
      sum.GetString(),
      sum.GetNumDigits());
...
BigInt FindLongestCollatzSequence(BigInt max)
{
  ...
  for (BigInt num = 1; num < max; ++num)
  {
    ...
    {
      printf(
          "Found an equal!  (%lld seq len %lld, vs %lld seq len %lld)\n",
          numOfLongest,
          longestSequence,
          num,
          len);
...
void RunLongestCollatzSequence(BigInt max)
{
  printf(
      ...
      max,
      FindLongestCollatzSequence(max));
}
...
BigInt CalcNumLatticePathsFromPoint(BigInt pointIndex, BigInt numPoints)
{
  ...
  const BigInt numLeftSidePaths =
      CalcNumLatticePathsFromPoint(pointIndex - 1, numPoints - 1);
  const BigInt numRightSidePaths =
      CalcNumLatticePathsFromPoint(pointIndex, numPoints - 1);
...
BigInt CalcNumLatticePaths(BigInt gridSize)
{
  ...
  {
    const BigInt numPathsFromPoint =
        CalcNumLatticePathsFromPoint(i, diagonalLength);
...
{
  printf(
      "Num paths through a grid of size %lld = %lld\n",
      gridSize,
      CalcNumLatticePaths(gridSize));
}
  ...
  if (numHundreds > 0)
  {
    ...
    if (verbose)
    {
      printf(
          "  %lld hundred (%lld)",
          numHundreds,
          s_numLettersTable_Under20[numHundreds] + 7);
    }
  }
    ...
    if (num >= 20)
    {
      ...
      if (verbose)
      {
        printf(
            ...
            tensPlace,
            under10,
            s_numLettersTable_Tens[tensPlace]
                + ((under10 > 0) ? s_numLettersTable_Under20[under10] : 0));
      }
    }
    else
    {
...
{
  printf(
      "The total number of letters in all numbers from 1 to %lld = %lld\n",
      max,
      CalcNumberLetterCount(max));
}
...
static BigInt s_maxPathSum1BigTri[] = {
  ...
  75, 3,  34, 88, 2,  77, 73, 7,  63, 67, 99, 65, 4,  28, 6,  16, 70, 92,
  ...
  48, 63, 66, 4,  68, 89, 53, 67, 30, 73, 16, 69, 87, 40, 31, 04, 62, 98,
...
BigInt CalcMaxPathFromTri(const BigInt* valueList, BigInt triHeight)
{
  ...
  printf("Winning path:  ");
  for (auto iter = theWinner.path.rbegin(); iter != theWinner.path.rend();
       ++iter)
  {
    printf("%lld ", *iter);
  }
...
{
  printf(
      "Max path from small tri = %lld\n",
      CalcMaxPathFromTri(s_maxPathSum1SmallTri, 4));
  printf(
      "Max path from big tri = %lld\n",
...
BigInt CountSundays()
{
  ...
  const BigInt numDaysIn1900 =
      IsLeapYear(1900) ? NUM_DAYS_IN_LEAP_YEAR : NUM_DAYS_IN_YEAR;
  ...
  for (BigInt year = 1901; year <= 2000; ++year)
  {
    ...
    for (BigInt month = JANUARY; month <= DECEMBER; ++month)
    {
      ...
      switch (month)
      {
        case FEBRUARY:
          day +=
              isLeapYear ? NUM_FEBRUARY_DAYS_IN_LEAP_YEAR : NUM_FEBRUARY_DAYS;
          break;
        case SEPTEMBER:
        case APRIL:
        case JUNE:
...
{
  printf(
      "There were %lld first-of-the-month Sundays in the 20th Century.\n",
      CountSundays());
}
...
{
  printf(
      "The sum of the digits of %lld! = %lld\n",
      num,
      CalcFactorialDigitSum(num));
}
...
{
  printf(
      "The sum of the proper divisors of %lld = %lld\n",
      num,
      CalcSumProperDivisors(num, true));
}

void TestSumAllAmicableNumbers(BigInt max)
{
  printf(
      "The sum of all amicable numbers under %lld = %lld\n",
      max,
      CalcSumAmicableNumbers(max));
}
...
void CalcNameValues(
    const std::vector<std::string>& nameList, std::vector<BigInt>& valueList)
{
...
void TestSingleNameScore(
    const std::vector<std::string>& nameList,
    const std::vector<BigInt>&      valueList,
    const char*                     name)
{
  ...
  printf(
      ...
      nameNumber,
      valueList[nameIndex],
      nameNumber * valueList[nameIndex]);
}

BigInt CalcSumNameScores(
    const std::vector<std::string>& nameList,
    const std::vector<BigInt>&      valueList)
{
  BigInt sum = 0;
...
void RunNamesScores()
{
  ...
  printf(
      "The sum of all name scores = %lld\n",
...
bool CanBeWrittenAsSumOfAbundantNumbers(
    BigInt num, const std::vector<bool>& abundantFlagList)
{
...
BigInt CalcSumCannotBeWrittenAsSumOfTwoAbundantNumbers(BigInt max)
{
  ...
  for (BigInt i = 1; i <= max; ++i)
  {
    const bool canBeWritten =
        CanBeWrittenAsSumOfAbundantNumbers(i, abundantFlagList);
    if (!canBeWritten)
    {
      sum += i;
...
bool RecurseLexicoPermut(
    char* begin, char* middle, char* end, std::function<bool(const char*)> func)
{
...
void RunLexicographicPermutations(
    const std::string& origSt, BigInt desiredPermut)
{
...
void RunNDigitFibonacci(BigInt numDigits)
{
  ...
  printf(
      "The first Fibonacci number to have %lld digits is:  Fib(%lld) = %s\n",
      numDigits,
      fibNum,
      num.GetString());
}
...
BigInt CalcNumQuadraticPrimes(BigInt coeffA, BigInt coeffB)
{
  ...
  for (;;)
  {
    const BigInt candidatePrime =
        (numPrimes * numPrimes) + (coeffA * numPrimes) + coeffB;
...
BigInt CalcCoeffProdQuadraticPrimes(BigInt coefficientRange)
{
  ...
  for (BigInt coeffA = -coefficientRange + 1; coeffA < +coefficientRange;
       ++coeffA)
  {
    printf("Testing coeff A = %lld\n", coeffA);
    for (BigInt coeffB = -coefficientRange; coeffB <= +coefficientRange;
         ++coeffB)
    {
...
void IterateDigitPowers(
    BigInt  numSoFar,
    BigInt  powerSumSoFar,
    BigInt  numDigitsLeft,
    BigInt  power,
    BigInt& sumValidNums)
{
  if (numDigitsLeft <= 0)
  ...
  for (BigInt digit = 0; digit <= 9; ++digit)
  {
    ...
    IterateDigitPowers(
        newNumSoFar, newPowerSumSoFar, newNumDigitsLeft, power, sumValidNums);
  }
}
...
void RunDigitPowers(BigInt power)
{
  ...
  printf(
      "Sum of all the numbers that can be written as the sum of %lld powers of their digits = %lld\n",
      power,
      sumValidNums);
}
...
static const BigInt s_numCoins =
    sizeof(s_coinValueList) / sizeof(s_coinValueList[0]);

void IterateCoinSums(
    BigInt* coinCountsList,
    BigInt  currSum,
    BigInt  goalSum,
    BigInt  coinIndex,
    BigInt& numWays)
...
{
  printf(
      "The coin sum %lld can be made in %lld different ways\n",
      sum,
      CalcCoinSumWays(sum));
}
...
void IteratePandigitalProducts(
    const char* sequence, BigInt& sum, std::set<BigInt>& set)
{
  for (BigInt numDigitsLeft = 1; numDigitsLeft <= 4; ++numDigitsLeft)
  {
    for (BigInt numDigitsRight = 1,
                numDigitsProd = 9 - numDigitsLeft - numDigitsRight;
         ...
         ++numDigitsRight, --numDigitsProd)
    {
      ...
      BigInt right = 0;
      BigInt prod = 0;
      CalcPandigitalSequenceNums(
          sequence, numDigitsLeft, numDigitsRight, left, right, prod);
      ...
      if (prod == left * right)
      {
        ...
        {
          printf(
              "Found valid prod:  %lld * %lld = %lld, but prod is already in set\n",
              left,
              right,
              prod);
        }
...
{
  printf(
      "The sum of pandigital products is %lld\n", CalcPandigitalProductSum());
}
...
BigInt CalcFactorialSum(BigInt num)
{
  static BigInt s_digitFactorial[] = {
    CalcFactorial(0), CalcFactorial(1), CalcFactorial(2), CalcFactorial(3),
    CalcFactorial(4), CalcFactorial(5), CalcFactorial(6), CalcFactorial(7),
    CalcFactorial(8), CalcFactorial(9),
  };
...
void RunDigitFactorials()
{
  ...
  printf(
      "Sum of all numbers equal to the sum of the factorials of their digits = %lld\n",
      sum);
}
...
{
  printf(
      "Num circular primes under %lld = %lld\n",
      max,
      CalcNumCircularPrimes(max));
}
...
{
  printf(
      "The sum of double base palindromes under %lld = %lld\n",
      max,
      CalcSumDoubleBasePalindromes(max));
}
...
{
  printf(
      "%lld is%s a truncatable prime!\n",
      num,
      IsTruncatablePrime(num) ? "" : " NOT");
}

void RunTruncatablePrimes(BigInt numTruncatables)
{
  printf(
      "The sum of the %lld first truncatable primes = %lld\n",
      numTruncatables,
      CalcSumTruncatablePrimes(numTruncatables));
}
...
BigInt CalcNumIntegerRightTriangleSolutions(BigInt perimeter)
{
  ...
  for (BigInt largeSide = minLargeSize; largeSide <= maxLargeSide; ++largeSide)
  {
    ...
    for (BigInt firstSmallSide = 1; firstSmallSide <= maxFirstSmallSide;
         ++firstSmallSide)
    {
      const BigInt secondSmallSide = leftOver - firstSmallSide;
      if ((firstSmallSide * firstSmallSide + secondSmallSide * secondSmallSide)
          == (largeSide * largeSide))
      {
        ++numSolutions;
        printf(
